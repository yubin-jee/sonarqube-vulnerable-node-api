...
const ALLOWED_CALLBACK_DOMAINS = ['webhook-service.internal', 'api.external-service.com'];
...
router.post('/register-webhook', async (req, res) => {
  ...
  try {
    ...
    if (!ALLOWED_CALLBACK_DOMAINS.includes(parsed.hostname)) {
      return res.status(400).json({ error: 'Callback URL domain not allowed' });
    }
  } catch (e) {
